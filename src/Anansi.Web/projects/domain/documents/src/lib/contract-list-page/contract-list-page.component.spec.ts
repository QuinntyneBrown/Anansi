...
describe('ContractListPageComponent', () => {
  ...
  it('should show error state on load failure', () => {
    ...
    expect(component.loading()).toBe(false);
    expect(component.error()).toBe('Failed to load contracts.');
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelector('.error-text').textContent).toContain('Failed to load contracts.');
  });

  it('should clear error on successful reload', () => {
    fixture.detectChanges();
    const req1 = httpTesting.expectOne(`${baseUrl}/api/contracts?isTemplate=false&page=1&pageSize=10`);
    req1.error(new ProgressEvent('Network error'));
    expect(component.error()).toBe('Failed to load contracts.');

    component.load();
    expect(component.error()).toBeNull();
    const req2 = httpTesting.expectOne(`${baseUrl}/api/contracts?isTemplate=false&page=1&pageSize=10`);
